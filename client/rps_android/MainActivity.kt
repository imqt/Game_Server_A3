...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.btnRock.setOnClickListener{
            ...
            try {
                thread {
                    socketHandler.make_move(1)
                }

                socketHandler.finalResponseArr = socketHandler.receiveFinal()
        ...
        binding.btnScissors.setOnClickListener{
            ...
            } catch (e:java.lang.Exception){error(e)}
            finally {
                disableButtons()
                while(socketHandler.getConfirmationCode() != 3);
                GlobalScope.launch {
    ...
    fun getWinnerText(): String {
        var finalResponse: ByteArray = socketHandler.finalResponseArr

        socketHandler.printByteArrayToString(finalResponse)

        var result = finalResponse[4].toInt()
        var gameResult: String?
        if (result == 1) { gameResult = "YOU WON"} else if (result == 2) { gameResult = "YOU LOST" } else { gameResult = "TIED GAME" }
        val otherPlayerMove = finalResponse[4].toInt()
